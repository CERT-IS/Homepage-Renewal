...
var s_finger_login = function (client_id,redirect_uri)
{
	this.button_color = BUTTON_COLOR_BLUE;
	this.button_type = BUTTON_TYPE;
	this.button_height = 40;
	this.nil_domain = "";
	this.response_type="token";
	this.authorize_url="http://api.s-finger.tk/oauth2.0/authorize";
	this.state = "";
	this.scope="";
...
/**
 * S Finger 로 로그인 버튼 디자인 선택
 * @ignore
 * @return void
 * @private
 */
	this.setButton = function(button_color,button_type,button_height) {
		this.button_color = ((typeof(button_color)!='undefined') && (button_color != "") ) ? button_color : BUTTON_COLOR_BLUE;
...
/**
 * 아이디로 로그인때 사용할 링크의 주소를 생성한다.
 * @ignore
 * @private
 */
	this.getSFingerLoginLink = function ()
	{
		if (!this.is_callback)
		{
			this.setStateStore();
...
/**
 * S Finger 로 로그인때 사용할 이미지 및 링크를 생성한다.
 * @ignore
 * @param {int} 이미지의 높이 값 (px 단위)
 * @private
 */
	this.init_s_finger_login = function ()
	{
		var s_finger_login = document.getElementById('s_finger_login');
		if (s_finger_login==undefined)
		{
			alert("id 가 s_finger_login 인 div tag 가 존재해야 합니다.");
			return false;
		}
		if (this.button_color=="blue")
		{
			color="b";
		}
		else
		...
		s_finger_login.innerHTML="";
		s_finger_login_contents="";
		s_finger_login_url = this.getSFingerLoginLink();
		if (this.state==undefined || this.state=="")
		{
			this.state = this.getUniqState();
		}
		s_finger_popup_option = "";
		if (this.popup)
		{
			s_finger_popup_option = " onClick=\"window.open(this.href, 'sfingerpopup', 'titlebar=1, resizable=1, scrollbars=yes, width=600, height=550'); return false\" ";
		}
		if (this.button_type == BUTTON_TYPE)
		{
			s_finger_login_contents="<a href='"+s_finger_login_url+"' "+s_finger_popup_option+" id='s_finger_login_anchor'><img src='https://developer.s-finger.tk/images/oauth/button_"+color+".png' border='0' title='Login with S Finger' width='"+this.button_height+"' height='"+this.button_height+"'></a> ";
		}
		else if (this.button_type == BANNER_SMALL_TYPE)
		{
			s_finger_login_contents="<a href='"+s_finger_login_url+"' "+s_finger_popup_option+" id='s_finger_login_anchor'><img src='https://developer.s-finger.tk/images/oauth/small_"+color+"_in.png' border='0' title='Login with S Finger' width='"+(this.button_height*656/250)+"px' height='"+this.button_height+"'></a> ";
		}
		else 
		{
			s_finger_login_contents="<a href='"+s_finger_login_url+"' "+s_finger_popup_option+" id='s_finger_login_anchor'><img src='https://developer.s-finger.tk/images/oauth/big_"+color+".png' border='0' title='Login with S Finger' width='"+(this.button_height*185/40)+"px' height='"+this.button_height+"px'></a> ";
		}
		s_finger_login.innerHTML=s_finger_login_contents;
		if (this.is_callback)
		{
			this.init_s_finger_callback();
		}
	}
 ...
 */
	...
	this.get_s_finger_userprofile = function(callback_func1) {
		$.ajax({
